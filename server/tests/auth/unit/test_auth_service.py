...
def test_build_authorization_url_and_state_should_return_right_url_and_state(
    ...
    monkeypatch.setattr(settings, "GITHUB_CLIENT_ID", "client_id_test")
    monkeypatch.setattr(
    ...
    # o state retornado precisa ser o mesmo que foi embutido na URL
    assert query["state"] == [state]
    assert len(state) > 0


@pytest.mark.unit()
def test_validate_state_should_return_true_for_valid_state(auth_service: AuthService):
...
def test_validate_state_should_return_false_for_invalid_state(
    ...
    assert auth_service.validate_state("state_from_github", "different_state") is False
...
async def test_callback_should_return_jwt_when_state_is_valid_and_flow_succeeds(
    ...
    monkeypatch.setattr(auth_service, "validate_state", MagicMock(return_value=True))
    monkeypatch.setattr(
        auth_service,
    ...
    monkeypatch.setattr(
        auth_service, "create_access_token", MagicMock(return_value="fake_jwt_token")
    )
...
async def test_callback_should_raise_http_exception_when_state_is_invalid(
    ...
    monkeypatch.setattr(auth_service, "validate_state", MagicMock(return_value=False))
    exchange_mock = AsyncMock()
    monkeypatch.setattr(auth_service, "exchange_code_for_token", exchange_mock)

    with pytest.raises(HTTPException) as exc_info:
        await auth_service.callback(
            code="fake_code", state="invalid_state", state_from_cookie="different_state"
        )
...
async def test_callback_should_raise_http_exception_when_user_id_is_missing(
    ...
    monkeypatch.setattr(auth_service, "create_access_token", create_token_mock)
    ...
    assert exc_info.value.status_code == 400
    assert exc_info.value.detail == "Failed to retrieve user information from GitHub."
